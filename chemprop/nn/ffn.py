...
class OneDEncoder(nn.Module):
    """
    对 X_d（一维指纹）做编码，输出一个较短的 embedding。
    可以用 MLP，也可以用 TransformerEncoder，这里先用 MLP。
    """

    def __init__(self, input_dim: int, hidden_dim: int = 512, output_dim: int = 256, dropout: float = 0.1, activation: nn.Module | None = None):
        super().__init__()
