...
class SpearmanCorrCoef(torchmetrics.Metric):
    ...
    def __init__(self, task_weights: ArrayLike = 1.0, **kwargs):
        ...
        """
        super().__init__()
        task_weights = torch.as_tensor(task_weights, dtype=torch.float).view(-1)
        self.register_buffer("task_weights", task_weights)
    ...
    def update(self, preds: Tensor, targets: Tensor, mask: Tensor | None = None, *args, **kwargs):
        ...
        mask = torch.ones_like(targets, dtype=torch.bool) if mask is None else mask.bool()
    ...
    def compute(self):
        if not self.preds:
            return torch.tensor(float("nan"), device=self.task_weights.device)

        preds = dim_zero_cat(self.preds).float()
        targets = dim_zero_cat(self.targets).float()
        masks = dim_zero_cat(self.masks).bool()
        if preds.ndim == 1:
            preds = preds.view(-1, 1)
        if targets.ndim == 1:
            targets = targets.view(-1, 1)
        ...
        for task_idx in range(targets.shape[1]):
            valid = masks[:, task_idx] & torch.isfinite(preds[:, task_idx]) & torch.isfinite(targets[:, task_idx])
            if int(valid.sum()) < 2:
                corrs.append(torch.tensor(float("nan"), device=preds.device))
                continue

            pred_rank = self._rankdata(preds[valid, task_idx])
            target_rank = self._rankdata(targets[valid, task_idx])
            pred_rank = pred_rank - pred_rank.mean()
            target_rank = target_rank - target_rank.mean()
            denom = pred_rank.norm() * target_rank.norm()
            corr = torch.tensor(float("nan"), device=preds.device) if denom <= 0 else (pred_rank * target_rank).sum() / denom
            corrs.append(corr)
        ...
        weights = self.task_weights.to(preds.device)
        if weights.numel() == 1:
            weights = weights.expand_as(corrs)
        weights = weights[: corrs.numel()]
        weights = weights[finite]
        return (corrs[finite] * weights).sum() / weights.sum()
