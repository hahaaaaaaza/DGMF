...
class _TaskWiseFFNPredictorBase(Predictor, HyperparametersMixin):
    """One MLP head per task for task-conditioned fused embeddings."""
    ...
    def __init__(
        ...
        self.heads = nn.ModuleList(
            [
                MLP.build(input_dim, self.n_targets, hidden_dim, n_layers, dropout, activation)
    ...
    def _validate_input(self, Z: Tensor) -> None:
        if Z.ndim != 3:
            raise ValueError(
                "Task-wise predictors expect Z with shape [batch, task, dim], "
                f"got {tuple(Z.shape)}."
            )
        if Z.shape[1] != self.n_tasks:
            raise ValueError(f"Expected {self.n_tasks} task embeddings, got {Z.shape[1]}.")

    def _raw_outputs(self, Z: Tensor) -> Tensor:
    ...
    def encode(self, Z: Tensor, i: int) -> Tensor:
        self._validate_input(Z)
        return torch.stack([head[:i](Z[:, j, :]) for j, head in enumerate(self.heads)], dim=1)


class TaskWiseRegressionFFN(_TaskWiseFFNPredictorBase):
    _T_default_criterion = MSE
    _T_default_metric = MSE

    def forward(self, Z: Tensor) -> Tensor:
        return self.output_transform(self._raw_outputs(Z))
...
class TaskWiseBinaryClassificationFFN(_TaskWiseFFNPredictorBase):
    _T_default_criterion = BCELoss
    _T_default_metric = BinaryAUROC

    def forward(self, Z: Tensor) -> Tensor:
        return self._raw_outputs(Z).sigmoid()

    def train_step(self, Z: Tensor) -> Tensor:
        return self._raw_outputs(Z)
