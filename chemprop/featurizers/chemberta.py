...
CHEMBERTA_DEFAULT_MODEL = "DeepChem/ChemBERTa-77M-MLM"


def resolve_chemberta_model_path(model_name: str) -> str:
    """Resolve a local ChemBERTa model path, tolerating a parent hf_models dir."""
...
def _canonical_smiles(smiles: str) -> str:
    ...
    if mol is None:
        raise ValueError(f"Invalid SMILES for ChemBERTa embedding: {smiles!r}")
    return Chem.MolToSmiles(mol, canonical=True, isomericSmiles=False)


def _chemberta_cache_path(
    ...
    return Path(cache_dir).expanduser() / "chemberta" / f"{digest}.pt"
...
def _load_cached_embedding(path: Path | None) -> np.ndarray | None:
    ...
    try:
        ...
        logger.warning("Failed to load ChemBERTa embedding cache %s: %s. Recomputing.", path, exc)
        return None
...
def _save_cached_embedding(path: Path | None, embedding: np.ndarray) -> None:
    ...
    try:
        ...
        logger.warning("Failed to save ChemBERTa embedding cache %s: %s", path, exc)


def _load_chemberta(model_name: str, device: str):
    try:
        import torch
        from transformers import AutoModel, AutoTokenizer
    except ImportError as exc:
        raise ImportError(
            "ChemBERTa 1D embeddings require the optional `transformers` dependency. "
            "Install it with `pip install -e .[molformer]` or `pip install transformers`."
        ) from exc

    model_name = resolve_chemberta_model_path(model_name)
...
def _pool_chemberta_output(outputs, attention_mask, pooling: str):
    if pooling == "pooler":
    ...
    raise ValueError(f"Unknown ChemBERTa pooling mode: {pooling}")


def chemberta_1d_embeddings(
    smiles: Sequence[str],
    model_name: str = CHEMBERTA_DEFAULT_MODEL,
    ...
    """Return frozen ChemBERTa-2 embeddings for SMILES strings."""

    if pooling not in {"pooler", "cls", "mean"}:
        raise ValueError("ChemBERTa pooling must be one of: pooler, cls, mean.")
    if max_length < 1:
        raise ValueError("ChemBERTa max_length must be at least 1.")
    if batch_size < 1:
        raise ValueError("ChemBERTa batch_size must be at least 1.")

    model_name = resolve_chemberta_model_path(model_name)
    canonical = [_canonical_smiles(smi) for smi in smiles]
    cache_paths = [
        _chemberta_cache_path(cache_dir, smi, model_name, max_length, pooling)
    ...
    if missing_indices:
        ...
        for start in range(0, len(missing_indices), batch_size):
            ...
            with torch.no_grad():
                ...
                pooled = _pool_chemberta_output(outputs, inputs["attention_mask"], pooling)
