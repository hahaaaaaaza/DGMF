...
def load_model(
    ...
    # Lightning `.ckpt` checkpoints are not the same format as our custom `.pt` files
    # (our `.pt` stores `hyper_parameters` + `state_dict` at the top level).
    is_ckpt = Path(path).suffix == ".ckpt"
    loader = model_cls.load_from_checkpoint if is_ckpt else model_cls.load_from_file
    return loader(path, map_location=torch.device("cpu"))
