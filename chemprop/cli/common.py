...
- ``RIGR``: Modified V2 (default) featurizer using only the resonance-invariant atom and bond features.""",
    ...
    featurization_args.add_argument(
        "--no-1d-fingerprints",
        action="store_true",
        help="Do not append native-dimensional MACCS, RDKit, and ECFP 1D fingerprints to datapoint descriptors.",
    )
    featurization_args.add_argument(
        "--use-molformer-1d",
        action="store_true",
        help="Use frozen MoLFormer SMILES embeddings as the 1D descriptor branch instead of native RDKit fingerprints.",
    )
    featurization_args.add_argument(
        "--trainable-molformer-1d",
        action="store_true",
        help=(
            "Use MoLFormer token features and fine-tune a MoLFormer encoder inside the "
            "model instead of precomputing frozen pooled embeddings."
        ),
    )
    featurization_args.add_argument(
        "--molformer-model",
        default="ibm-research/MoLFormer-XL-both-10pct",
        help="HuggingFace model id or local path for frozen MoLFormer embeddings.",
    )
    featurization_args.add_argument(
        "--molformer-cache-dir",
        default=None,
        help="Optional directory for molecule-level cached MoLFormer embedding .pt files.",
    )
    featurization_args.add_argument(
        "--molformer-device",
        default="cpu",
        help="Device used while computing missing MoLFormer embeddings, e.g. cpu or cuda.",
    )
    featurization_args.add_argument(
        "--molformer-max-length",
        type=int,
        default=256,
        help="Maximum tokenized SMILES length for MoLFormer.",
    )
    featurization_args.add_argument(
        "--molformer-pooling",
        choices=("pooler", "cls", "mean"),
        default="pooler",
        help="Pooling strategy for MoLFormer hidden states.",
    )
    featurization_args.add_argument(
        "--molformer-batch-size",
        type=int,
        default=32,
        help="Batch size used while computing missing MoLFormer embeddings.",
    )
    featurization_args.add_argument(
        "--use-chemberta-1d",
        action="store_true",
        help="Use frozen ChemBERTa-2 SMILES embeddings as the 1D descriptor branch instead of native RDKit fingerprints.",
    )
    featurization_args.add_argument(
        "--chemberta-model",
        default="DeepChem/ChemBERTa-77M-MLM",
        help="HuggingFace model id or local path for frozen ChemBERTa-2 embeddings.",
    )
    featurization_args.add_argument(
        "--chemberta-cache-dir",
        default=None,
        help="Optional directory for molecule-level cached ChemBERTa embedding .pt files.",
    )
    featurization_args.add_argument(
        "--chemberta-device",
        default="cpu",
        help="Device used while computing missing ChemBERTa embeddings, e.g. cpu or cuda.",
    )
    featurization_args.add_argument(
        "--chemberta-max-length",
        type=int,
        default=256,
        help="Maximum tokenized SMILES length for ChemBERTa.",
    )
    featurization_args.add_argument(
        "--chemberta-pooling",
        choices=("pooler", "cls", "mean"),
        default="mean",
        help="Pooling strategy for ChemBERTa hidden states.",
    )
    featurization_args.add_argument(
        "--chemberta-batch-size",
        type=int,
        default=32,
        help="Batch size used while computing missing ChemBERTa embeddings.",
    )
    featurization_args.add_argument(
        "--use-unimol-3d",
        action="store_true",
        help=(
            "Use frozen Uni-Mol molecule-level CLS embeddings as descriptor-style 3D features. "
            "The embeddings are prepended to X_d and should be paired with --x-d-3d-dim."
        ),
    )
    featurization_args.add_argument(
        "--unimol-model-name",
        default="unimolv1",
        help="Uni-Mol model name passed to unimol_tools.UniMolRepr.",
    )
    featurization_args.add_argument(
        "--unimol-model-path",
        default=None,
        help="Optional local Uni-Mol checkpoint/model path when supported by unimol_tools.",
    )
    featurization_args.add_argument(
        "--unimol-cache-dir",
        default=None,
        help="Optional directory for molecule-level cached Uni-Mol embedding .npy files.",
    )
    featurization_args.add_argument(
        "--unimol-device",
        default="cpu",
        help="Device used while computing missing Uni-Mol embeddings, e.g. cpu or cuda.",
    )
    featurization_args.add_argument(
        "--unimol-batch-size",
        type=int,
        default=32,
        help="Batch size used while computing missing Uni-Mol embeddings.",
    )
    featurization_args.add_argument(
        "--unimol-remove-hs",
        action="store_true",
        help="Ask Uni-Mol to remove hydrogens during representation extraction when supported.",
    )
    featurization_args.add_argument(
        "--use-mmff-3d-graph",
        action="store_true",
        help="Generate an MMFF conformer geometry graph and encode it with the 3D graph transformer branch.",
    )
    featurization_args.add_argument(
        "--use-mace-3d-graph",
        action="store_true",
        help="Generate a MACE-optimized conformer and use MACE atomic descriptors as the 3D graph node features.",
    )
    featurization_args.add_argument(
        "--use-gotennet-3d-graph",
        action="store_true",
        help="Generate an MMFF conformer geometry graph and encode it with a trainable GotenNet 3D backbone.",
    )
    featurization_args.add_argument(
        "--mace-model",
        default="medium",
        help="MACE-OFF foundation model size/name used when --use-mace-3d-graph is enabled.",
    )
    featurization_args.add_argument(
        "--mace-model-path",
        default=None,
        help="Optional local MACE checkpoint path. If provided, it overrides --mace-model.",
    )
    featurization_args.add_argument(
        "--mace-device",
        default="cpu",
        help="Device for MACE inference/optimization, e.g. cpu or cuda.",
    )
    featurization_args.add_argument(
        "--mace-dtype",
        choices=("float32", "float64"),
        default="float32",
        help="Floating point precision used by the MACE calculator.",
    )
    featurization_args.add_argument(
        "--mace-no-optimize",
        action="store_true",
        help="Skip MACE geometry optimization and only use MACE for atomic descriptor extraction.",
    )
    featurization_args.add_argument(
        "--mace-fmax",
        type=float,
        default=0.03,
        help="Force convergence threshold for MACE geometry optimization.",
    )
    featurization_args.add_argument(
        "--mace-num-layers",
        type=int,
        default=None,
        help="Optional number of MACE layers to use when extracting descriptors.",
    )
    featurization_args.add_argument(
        "--mace-cache-dir",
        default=None,
        help=(
            "Optional directory for cached MACE 3D geometry graphs. When set, repeated runs "
            "with the same input SMILES and MACE settings reuse precomputed graphs."
        ),
    )
    featurization_args.add_argument(
        "--geometry-cache-dir",
        default=None,
        help=(
            "Optional directory for molecule-level cached 3D geometry graphs. For MMFF/GotenNet, "
            "each cache file stores only the lowest-energy optimized conformer graph."
        ),
    )
    featurization_args.add_argument(
        ...
        default=1,
        help=(
            "Number of RDKit ETKDG conformer candidates to optimize and screen before keeping the "
            "lowest-energy 3D graph for MMFF/GotenNet."
        ),
    )
    featurization_args.add_argument(
...
def validate_common_args(args):
    ...
    if args.geometry_num_conformers < 1:
        raise ArgumentError(
            argument=None,
            message="`--geometry-num-conformers` must be at least 1.",
        )
    if args.use_molformer_1d and args.no_1d_fingerprints:
        raise ArgumentError(
            argument=None,
            message="`--use-molformer-1d` conflicts with `--no-1d-fingerprints`.",
        )
    if args.use_chemberta_1d and args.no_1d_fingerprints:
        raise ArgumentError(
            argument=None,
            message="`--use-chemberta-1d` conflicts with `--no-1d-fingerprints`.",
        )
    if args.use_molformer_1d and args.use_chemberta_1d:
        raise ArgumentError(
            argument=None,
            message="Use only one 1D language-model branch: `--use-molformer-1d` or `--use-chemberta-1d`.",
        )
    if args.trainable_molformer_1d and not args.use_molformer_1d:
        raise ArgumentError(
            argument=None,
            message="`--trainable-molformer-1d` requires `--use-molformer-1d`.",
        )
    if args.trainable_molformer_1d and not args.no_descriptor_scaling:
        logger.warning(
            "`--trainable-molformer-1d` carries token ids in X_d; forcing "
            "`--no-descriptor-scaling`."
        )
        args.no_descriptor_scaling = True
    if args.molformer_max_length < 1:
        raise ArgumentError(
            argument=None,
            message="`--molformer-max-length` must be at least 1.",
        )
    if args.molformer_batch_size < 1:
        raise ArgumentError(
            argument=None,
            message="`--molformer-batch-size` must be at least 1.",
        )
    if args.chemberta_max_length < 1:
        raise ArgumentError(
            argument=None,
            message="`--chemberta-max-length` must be at least 1.",
        )
    if args.chemberta_batch_size < 1:
        raise ArgumentError(
            argument=None,
            message="`--chemberta-batch-size` must be at least 1.",
        )
    if args.unimol_batch_size < 1:
        raise ArgumentError(
            argument=None,
            message="`--unimol-batch-size` must be at least 1.",
        )
