...
class _MolGraphDatasetMixin:
    ...
    @property
    def X_3d(self) -> list[GeometryGraph | None]:
        return [d.x_3d for d in self.data]
...
class CuikmolmakerDataset(MoleculeDataset):
    ...
    def __getitems__(self, indexes: list[int]) -> CuikBatchedDatum:
        ...
        X_3d = [self.X_3d[idx] for idx in indexes] if self.X_3d[0] is not None else None
        ...
        return CuikBatchedDatum(bmg, V_d, X_d, X_3d, Y, weights, lt_mask, gt_mask)
