...
$VariantRoot = Join-Path $OutputRoot "matched_shared_target_agnostic"
$LogRoot = Join-Path $ReleaseRoot "logs"
$LogPath = Join-Path $LogRoot "matched_shared_target_agnostic_20260814.combined.log"
...
& $Python (Join-Path $PSScriptRoot "audit_matched_shared_target_agnostic.py") `
...
& $Python (Join-Path $PSScriptRoot "verify_matched_shared_target_agnostic_results.py") `
    --variant-root $VariantRoot 2>&1 | Tee-Object -FilePath $LogPath -Append
